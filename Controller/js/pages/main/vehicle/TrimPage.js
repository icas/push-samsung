TrimPage = movl.ui.View.extend({
	
	initialize: function(options) {
		movl.ui.View.prototype.initialize.call(this, options);
		
		App.logger.debug('initialize()');
		this.el = $($('#trimPageTemplate', App.templates).html());
		
		// el set by MainPage
		this.delegateEvents();
		
		this.selections = App.data.getTrimSelections(App.data.selectedData);
		App.data.logList(this.selections, 'TrimPage: selections');
	...
	events: {
		'click #sliderContainer' : 'onSelectTrim',
		'click #next' : 'onNext',
		'click #prev' : 'onPrev'
	},
	...
	preloadSelectionsImages: function(selections, callback) {
		App.logger.debug('TrimPage.preloadSelectionsImages()');
		var images = [];
		for(var i = 0; i < selections.length; i++) {
			var selection = selections[i];
			var imagePath = App.config.imagesPath + selection.MovlImageName;
		...
		preloader.bind('complete', _.bind(function() {
			App.logger.debug('TrimPage.preloadSelectionsImages() COMPLETE');
			//App.kontrolTV.application.alert('TrimPage.preloadSelectionsImages() COMPLETE');
			callback();
		}, this));
	...
	activate: function() {
		...
		App.logger.debug('TrimPage.activate()');
